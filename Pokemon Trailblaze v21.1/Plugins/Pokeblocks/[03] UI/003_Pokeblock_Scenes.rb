...
class PokeblockCase_Scene
	...
	def pbRefresh
		...
		@sprites["background"].setBitmap(sprintf("Graphics/UI/Pokeblock/UI Pokeblock/pokeblock_background"))
		pbDrawImagePositions(@sprites["background"].bitmap,[["Graphics/UI/Pokeblock/UI Pokeblock/pokeblock_bg_lv",Graphics.width-50,0]]) if !SIMPLE
	...
	def pbDrawBlockInfo(block)
		...
		5.times { |i|	
			x = i < 3 ? 8 : 104
			y = 18 + (i % 3) * 32
			imagepos.push(["Graphics/UI/Pokeblock/Blocks/"+arr[i],x,y]) if block.flavor[i] > 0 if block 
		...
		item.clear
		imagepos.push(["Graphics/UI/Pokeblock/Blocks/Block_"+GameData::PokeblockColor.get(block.color).name+(block.plus ? "_Plus" : ""),0,0]) if block 
...
class PokeblockCondition_Scene
	...
	def pbRefresh
		# Set the background image
		file = "Graphics/UI/Pokeblock/UI Pokeblock/condition_background_m" +(useBarGraph ? "" : "_graph")+ ((SIMPLE || NO_SHEEN) ? "_simple" : "")
	...
	def pbDrawPokemonInfo
		...
		if useBarGraph
			6.times { |i|
				...
				maxAdjust = 8 if i == 5
				imagepos.push(["Graphics/UI/Pokeblock/UI Pokeblock/max", xBase+56+maxAdjust, y-20]) if fea[i] >=255
			}
		else				
			@sprites["statsgraph"].bitmap.clear
			graphY = yBase+110
			...
			if !(SIMPLE || NO_SHEEN)
				...
				set_xy_sprite("Sheen bar", x, y)
				maxAdjust = 8
				imagepos.push(["Graphics/UI/Pokeblock/UI Pokeblock/max", xBase+56+maxAdjust, y-20]) if fea[5] >=255
			end
		end
		# Scene pokemon
		# x = 470
...
class PokeblockEat_Scene
	...
	def pbStartScene
		...
		@sprites = {}
		@sprites["background"] = IconSprite.new(0, 0, @viewport)
		@sprites["background"].setBitmap(sprintf("Graphics/UI/Pokeblock/UI Pokeblock/eat_scene"))
		#Pokemon
		@sprites["pokemon"] = PokemonSprite.new(@viewport)
		@sprites["pokemon"].setOffset(PictureOrigin::CENTER)
		# @sprites["pokemon"].x = 104
...
class PokeblockKit_Scene
	...
	def pbStartScene(commands)
		...
		@sprites = {}
		@sprites["background"] = IconSprite.new(0, 0, @viewport)
		@sprites["background"].setBitmap("Graphics/UI/Pokeblock/UI Pokeblock/kit_background")
		@commands.length.times do |i|
...
class PokeblockKitButton < Sprite
  ...
  def initialize(command, x, y, viewport = nil)
    ...
    @name  = command
    @selected = false
    @button = AnimatedBitmap.new("Graphics/UI/Pokeblock/UI Pokeblock/kit_button")
...
class MultiBerrySelection_Scene
  ...
  def pbRefresh
    # Set the background image
    @sprites["background"].setBitmap(sprintf("Graphics/UI/Pokeblock/UI Berry Blender/SimpleBerrySelection"))
    # Refresh the item window
  ...
  def drawSelectedBerryCircles
	imagepos = []
	4.times { |i| imagepos.push(["Graphics/UI/Pokeblock/UI Pokeblock/berry_select_circle", 25 + (i%2 != 0 ? 75 : 0), 70 + (i>1 ? 75 : 0)]) }
