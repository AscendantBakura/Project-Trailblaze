...
class PokemonSummary_Scene
		...
		if !@contestpage
			...
			if BWSUMMARY
				if SUMMARY_B2W2_STYLE
					@sprites["menuoverlay"].setBitmap("Graphics/UI/Summary/bg_3_contest_b2w2")
				else
					@sprites["menuoverlay"].setBitmap("Graphics/UI/Summary/bg_3_contest_bw")
				end
			else
				@sprites["background"].setBitmap(sprintf("Graphics/UI/Summary/bg_3_contest")) 
			end
		...
		if !@contestpage
			...
			Pokemon::MAX_MOVES.times do |i|
			  ...
			  if move
				...
				imagepos.push(["Graphics/UI/Contest/contesttype", xPos, yPos + yAdj - 4, 0, type_number * 28, 64, 28])
	...
	def drawSelectedContestMove(move_to_learn, selected_move)
		...
		imagepos.push(["Graphics/UI/Contest/move_heart#{hearts}", 166, 124]) if hearts > 0
		imagepos.push(["Graphics/UI/Contest/move_negaheart#{jam}", 166, 156]) if jam > 0
		pbDrawImagePositions(overlay, imagepos)
	...
	def pbCreateConditionBars(simple,noSheen,bargraph)
		...
		if bargraph
			6.times { |i|
				...
				textpos = []
				barBitmap = AnimatedBitmap.new("Graphics/UI/Pokeblock/UI Pokeblock/Bar_#{arr[i]}")
				...
				imagepos.push(["Graphics/UI/Pokeblock/UI Pokeblock/Bar_bg",xBase,yBase + 48 * i])
				imagepos.push(["Graphics/UI/Pokeblock/UI Pokeblock/max", xBase+string_width+4, @sprites["#{arr[i]} bar"].y-20]) if fea[i] >=255
			}
		else				
			...
			5.times { |i|
				...
				imagepos.push(["Graphics/UI/Contest/contesttype", x, y, 0, i * 28, 64, 28])
				if fea[i] >= 255
					x = [400,420,464,336,298][i]
					x += xBaseAdj
					y = [118,176,302,302,176][i]
					y -= ((simple || noSheen) ? 0 : 16)
					imagepos.push(["Graphics/UI/Pokeblock/UI Pokeblock/max", x, y])
				end
			}
			if !(simple || noSheen)	
				barBitmap = AnimatedBitmap.new("Graphics/UI/Pokeblock/UI Pokeblock/Bar_Sheen")
				...
				imagepos.push(["Graphics/UI/Pokeblock/UI Pokeblock/Bar_bg",xBase,yBase + 48 * 5])
				imagepos.push(["Graphics/UI/Pokeblock/UI Pokeblock/max", xBase+string_width+4, @sprites["Sheen bar"].y-20]) if fea[5] >=255
  ...
  def pbDrawMoveList
    ...
    VISIBLEMOVES.times do |i|
      ...
      if moveobject
		...
		if @contestinfo
			imagepos.push(["Graphics/UI/Contest/contesttype", 12, yPos - 4, 0, contest_type_number * 28, 64, 28])
		else
    ...
    pbDrawTextPositions(overlay, textpos)
	if @contestinfo
		imagepos.push(["Graphics/UI/Contest/move_heart#{hearts}", 436, 116]) if hearts > 0
		imagepos.push(["Graphics/UI/Contest/move_negaheart#{jam}", 436, 148]) if jam > 0
	else
