...
class PokemonContestTalent_Scene
	...
	def pbShowApplauseMeter
		timer = System.uptime
		loop do
			@sprites["applausemeter"].x = lerp(0, @sprites["applausemeter"].bitmap.width, 0.3, timer, System.uptime) - @sprites["applausemeter"].bitmap.width
			@sprites["applausemeteroverlay"].x = @sprites["applausemeter"].x
			Graphics.update
			Input.update
			pbUpdate
			break if @sprites["applausemeter"].x >= 0
		end
		@sprites["applausemeter"].x = 0
		@sprites["applausemeteroverlay"].x = @sprites["applausemeter"].x
	end
	...
	def pbHideApplauseMeter
		timer = System.uptime
		loop do
			@sprites["applausemeter"].x = -1 * lerp(0, @sprites["applausemeter"].bitmap.width, 0.3, timer, System.uptime)
			@sprites["applausemeteroverlay"].x = @sprites["applausemeter"].x
			Graphics.update
			Input.update
			pbUpdate
			break if @sprites["applausemeter"].x <= -@sprites["applausemeter"].bitmap.width
		end
		@sprites["applausemeter"].x = -@sprites["applausemeter"].bitmap.width
		@sprites["applausemeteroverlay"].x = @sprites["applausemeter"].x
	end
	...
	def pbWaitCrowd(second)
		...
		loop do
			...
			pbUpdateSceneMap
			updateCrowdAnim
			break if System.uptime - timer >= second
		end
	end
	...
	def pbStartRound
		...
		timer = System.uptime
		while @sprites["curtain"].y > -@sprites["curtain"].bitmap.height
			@sprites["curtain"].y = -1 * lerp(0, @sprites["curtain"].bitmap.height, 0.5, timer, System.uptime)
			Graphics.update
			Input.update
			pbUpdate
		end
		@sprites["curtain"].y = -@sprites["curtain"].bitmap.height
		pbWait(0.2)
		pbShowApplauseMeter
	...
	def pbMoveShowDetails(move)
		...
		imagepos1 << ["Graphics/UI/Contest/contesttype", 206, 204, 0, type * 28, 64, 28]
	...
	def pbEndRound
		...
		while @sprites["curtain"].y < 0
			@sprites["curtain"].y = lerp(0, @sprites["curtain"].bitmap.height, 0.5, timer, System.uptime) - @sprites["curtain"].bitmap.height
			Graphics.update
			Input.update
			pbUpdate
		end
