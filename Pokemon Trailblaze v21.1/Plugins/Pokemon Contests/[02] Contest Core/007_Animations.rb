...
class PokemonContestTalent_Scene
	def pbDisplayPokemon(position)
		...
		timer = System.uptime
		old_x = @sprites["pokemonsprite#{position}"].x
		loop do
			@sprites["pokemonsprite#{position}"].x = old_x - lerp(0, 200, 0.5, timer, System.uptime)
			Graphics.update
			Input.update
			pbUpdateSpriteHash(@sprites)
			break if @sprites["pokemonsprite#{position}"].x <= old_x - 200
		end
		@sprites["pokemonsprite#{position}"].x = old_x - 200
	end
