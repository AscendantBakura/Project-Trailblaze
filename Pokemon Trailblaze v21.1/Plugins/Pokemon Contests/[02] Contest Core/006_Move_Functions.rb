...
class PokemonContestTalent_Scene
	...
	def pbUseMove(position)
		...
		if pkmn.c_nomoremoves || !move
			pbContestMessage(_INTL("{1} watches the others.\\wtnp[10]", pkmn.name))
			noMove = true
		
		elsif pkmn.c_missturn
			pbContestMessage(_INTL("{1} took a break.\\wtnp[10]", pkmn.name))
			pkmn.c_missturn = false
			noMove = true
			
		else
			if pkmn.c_nervous
				pbContestMessage(_INTL("{1} is nervous.\\wtnp[10]", pkmn.name))
				# Check for nervousness, 30% base chance
				chance = 30
				chance += 15 if pkmn.c_hasattention
				chance -= 5 * pkmn.c_spirit
				if rand(100)<chance
					pbContestMessage(_INTL("{1} is too nervous to move!\\wtnp[10]", pkmn.name))
					noMove = true
				else
					...
					pbContestMessage(_INTL("{1} tried to show its appeal with {2}!\\wtnp[10]", pkmn.name, move_name))
		...
		if noMove
			...
			if round > 1 && move && !move.canBeUsedRepeatedly? && pkmn.c_lastmove && pkmn.c_currentmove && pkmn.c_currentmove == pkmn.c_lastmove
				...
				pbContestMessage(_INTL("{1} disappointed the Judge by repeating the move.\\wtnp[10]", pkmn.name))
			...
			if round > 1 && pkmn.c_hasattention && move && pkmn.checkContestCombos(move)
				...
				pbContestMessage(_INTL("The combo went over well!\\wtnp[10]"))
				pkmn.c_hasattention = false
				pbApplyHeartsChange(pkmn)
			...
			if round < 5 && move && move.startsContestCombo 
				...
				pbContestMessage(_INTL("Anticipation is swelling for a combo on the next turn!\\wtnp[10]"))
			...
			if @contest.isCrowdPaused?(pkmn) || move.is_neutral_category?(@contest.category)
				...
				pbContestMessage(_INTL("The audience settled down.\\wtnp[10]"))
				updateApplauseMeter
				pbWait(0.5)
				...
				if @contest.crowdEnergy >= 5
					...
					pbContestMessage(_INTL("{1}'s {2} got the audience going!\\wtnp[10]", pkmn.name,pbContestCatName(@contest.category)))
					...
					if pkmn.c_allowmega && !pkmn.c_mega
						megaEvolve(pkmn)
						pbContestMessage(_INTL("{1} Mega Evolved and moved the entire audience in the hall!\\wtnp[10]", pkmn.name))
						pkmn.c_pending_hearts += 2
						pbWait(0.1)
					end
					pbApplyHeartsChange(pkmn)
					...
					pbContestMessage(_INTL("{1}'s {2} really excited the audience!\\wtnp[10]", pkmn.name,pbContestCatName(move.contest_type_position)))
					pbSEPlay("Contest crowd")
					animateCrowd
					updateApplauseMeter
					pbWaitCrowd(0.1)
				...
				if move.getContestFunction.better_if_increases_crowd
					...
					pbContestMessage(_INTL("Exciting the audience improved {1}'s appeal even more!\\wtnp[10]", pkmn.name))
					pbWaitCrowd(0.1)
					pbApplyHeartsChange(pkmn)
				end
				...
				pkmn.c_pending_hearts -= 1
				pbContestMessage(_INTL("{1}'s show of {2} didn't go over very well with this audience.\\wtnp[10]", pkmn.name,pbContestCatName(move.contest_type_position)))
				updateApplauseMeter
				pbWait(0.1)
				pbApplyHeartsChange(pkmn)
				pbWait(0.5)
	...
	def pbApplyMoveEffects(pkmn, move, position)
		...
		if func.startles
			...
			pbContestMessage(_INTL("It tried to startle the other Pokémon!\\wtnp[10]"))
			if affected == 0
				pbContestMessage(_INTL("But it failed!\\wtnp[10]"))
			else
				for i in 0...position-1
					p = @contest.roundOrder[i]
					next if !p.c_startled
					if p.c_oblivious
						pbContestMessage(_INTL("{1} managed to avoid seeing it.\\wtnp[10]", p.name))
						p.c_startled = false
						pbSEPlay("Contest status effect remove")
						pbUpdateContestantInfo(p)
					elsif p.c_calm
						pbContestMessage(_INTL("{1} managed to avert its gaze.\\wtnp[10]", p.name))
						p.c_startled = false
						p.c_calm = false
						pbSEPlay("Contest status effect remove")
						pbUpdateContestantInfo(p)
						...
						if p.c_pending_hearts >= -1
							pbContestMessage(_INTL("{1} looked down out of distraction.\\wtnp[10]", p.name))
						elsif p.c_pending_hearts >= -3
							pbContestMessage(_INTL("{1} turned back out of distraction.\\wtnp[10]", p.name))
						else
							pbContestMessage(_INTL("{1} couldn't help leaping up.\\wtnp[10]", p.name))
						end
						pbWait(0.1)
		...
		if func.unnerve
			if position == 4
				pbContestMessage(_INTL("It tried to unnerve the other Pokémon!\\wtnp[10]"))
				pbContestMessage(_INTL("But it failed!\\wtnp[10]"))
			elsif scope == "All"
				for i in position..3
					@contest.roundOrder[i].c_nervous = true
				end
				pbContestMessage(_INTL("It tried to unnerve the Pokémon waiting for their turns!\\wtnp[10]"))
				pbUpdateAllContestantInfo
			elsif scope == "One"
				@contest.roundOrder[position].c_nervous = true
				pbUpdateAllContestantInfo
				pbContestMessage(_INTL("It tried to unnerve the next Pokémon waiting for its turn!\\wtnp[10]"))
			end
		end
		#User Status
		if func.double_next_appeal
			pkmn.c_doublenext = true
			pbContestMessage(_INTL("It started getting prepared for its next appeal.\\wtnp[10]"))
		end
		if func.increase_spirit
			if pkmn.c_spirit < 3
				pkmn.c_spirit += 1
				pbContestMessage(_INTL("{1} got even more pumped up than usual!\\wtnp[10]", pkmn.name))
				pbSEPlay("Contest spirit up")
				pbUpdateContestantInfo(pkmn)
			else
				pbContestMessage(_INTL("{1}'s spirit won't go any higher!\\wtnp[10]", pkmn.name))
			end
		end
		if func.no_more_moves
			pkmn.c_nomoremoves = true
			pbContestMessage(_INTL("{1} will be unable to use any moves after this!\\wtnp[10]", pkmn.name))
			pbUpdateContestantInfo(pkmn)
		end
		if func.easily_startled
			pkmn.c_easilystartled = true
			pbContestMessage(_INTL("It became very aware of the Pokémon going on after it.\\wtnp[10]"))
		end
		if func.avoid_startled
			pkmn.c_oblivious = true
			pbContestMessage(_INTL("It is completely oblivious to the other Pokémon's moves.\\wtnp[10]"))
			pbUpdateContestantInfo(pkmn)
		end
		if func.avoid_startled_once
			pkmn.c_calm = true
			pbContestMessage(_INTL("It is feeling pretty calm and collected now.\\wtnp[10]"))
			pbUpdateContestantInfo(pkmn)
		end		
		#Changes Crowd
		if func.pause_crowd
			if move.is_positive_category?(@contest.category) && !@contest.isCrowdPaused?(pkmn)
				...
				pbContestMessage(_INTL("{1} attracted the audience's full attention!\\wtnp[10]", pkmn.name))
			else
				pbContestMessage(_INTL("{1} tried to dazzle the audience!\\wtnp[10]", pkmn.name))
				pbContestMessage(_INTL("But it failed!\\wtnp[10]"))
			end
		end
		#Depends on Crowd
		if func.depends_on_crowd
			...
			if scope == "Equal" && crowd > 0
				...
				pbContestMessage(_INTL("{1} matched the audience's excitement!\\wtnp[10]", pkmn.name))
			elsif scope == "Higher"
				case crowd
				when 0
					pbContestMessage(_INTL("{1} didn't show off its appeal well.\\wtnp[10]", pkmn.name))
				when 1..2
					pkmn.c_pending_hearts += 2
					pbContestMessage(_INTL("{1} showed off a little of its appeal.\\wtnp[10]", pkmn.name))
				when 3
					pkmn.c_pending_hearts += 3
					pbContestMessage(_INTL("{1} showed off its appeal pretty well!\\wtnp[10]", pkmn.name))
				when 4
					pkmn.c_pending_hearts += 5
					pbContestMessage(_INTL("{1} showed off its appeal fantastically well!\\wtnp[10]", pkmn.name))
				end
			elsif scope == "Lower"
				case crowd
				when 4
			...
			if ["Lower","Poor","Good"].include?(scope) && other
				if other <= 2
					if scope == "Poor" || scope == "Lower"
						pkmn.c_pending_hearts += 6
						pbContestMessage(_INTL("{1} totally outshone the last Pokémon! What appeal!\\wtnp[10]", pkmn.name))
					elsif scope == "Good"
						pbContestMessage(_INTL("{1} didn't show off its appeal well.\\wtnp[10]", pkmn.name))
					end
				elsif other <=3
					if scope == "Poor" || scope == "Good" 
						...
						pkmn.c_pending_hearts += 3
						pbContestMessage(_INTL("{1} was on par with the last Pokémon.\\wtnp[10]", pkmn.name))
					end
				else
					if scope == "Poor" || scope == "Lower" 
						pbContestMessage(_INTL("{1} didn't show off its appeal well.\\wtnp[10]", pkmn.name))
						...
						pkmn.c_pending_hearts += 6
						pbContestMessage(_INTL("{1} showed off as well as the last Pokémon! What appeal!\\wtnp[10]", pkmn.name))
					end
				end				
			elsif scope == "Equal Half" && other
				pkmn.c_pending_hearts = (other/2).round - move.contest_hearts
				pbContestMessage(_INTL("The previous appeal influenced {1}'s appeal!\\wtnp[10]", pkmn.name))
		...
		if func.decrease_other_spirit
			pbContestMessage(_INTL("It tried to taunt the Pokémon that are feeling pumped up!\\wtnp[10]"))
			if scope == "Before"
				if position == 1
					pbContestMessage(_INTL("But it failed!\\wtnp[10]"))
				else
					...
					pbContestMessage(_INTL("But it failed!\\wtnp[10]")) if affected == 0
				end
			elsif scope == "After"
				if position == 4
					pbContestMessage(_INTL("But it failed!\\wtnp[10]"))
		...
		if func.scramble_order
			...
			pbContestMessage(_INTL("{1} scrambled the order for the next turn!\\wtnp[10]", pkmn.name))
			pbUpdateAllContestantInfo
		end
		if func.do_next_earlier && !@contest.scramble
			...
			pbContestMessage(_INTL("{1} moved up in line so that it can go earlier next turn.\\wtnp[10]", pkmn.name))
			pbUpdateAllContestantInfo
		end
		if func.do_next_later && !@contest.scramble
			...
			pbContestMessage(_INTL("{1} moved back in line so that it can go later next turn.\\wtnp[10]", pkmn.name))
			pbUpdateAllContestantInfo
		end
		#Steal attention
		if func.steal_attention
			...
			#Anticipation Judge
			pbShowJudgeIcon("potential")
			pbContestMessage(_INTL("{1} stole the Judge's attention from others!\\wtnp[10]", pkmn.name))
		end
		...
		if func.skip_next_move
			...
			pbContestMessage(_INTL("{1} can't appeal next turn!\\wtnp[10]", pkmn.name))
		end
	end
