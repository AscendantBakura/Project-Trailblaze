...
STARTER_CHOICE_OUTCOME_VARIABLE = 0
  ...
  def pbChooseStarter
    loop do
      ...
      if Input.trigger?(Input::LEFT) || Input.triggerex?(0x41)
        ...
        if pbConfirmMessage(_INTL("Are you sure you want to choose {1}?", pokemon_name))
          pbPlayDecisionSE
          @confirmed = true
          return @index
        end
...
def pbChooseStarter(starters, level = 5, allow_cancel = true, title = nil, can_save = true, state_key = nil)
  ...
  if index >= 0
    ...
    if is_shiny
      pbMessage(_INTL("You received a shiny {1}!", pokemon.name))
    else
